...
public class BinarySearch {
    ...
    public static void main(String[] args) {
        In in = new In(args[0]);
        String s = in.readAll();
        String[] words = s.split("\\s+"); //delar upp på ord
        System.err.println("Done reading words"); //err visar för användaren (rött)
        ...
        while (!StdIn.isEmpty()) {
            String key = StdIn.readString(); //läser en rad från användaren
            if (search(key, words) < 0) StdOut.println(key); //om key inte hittades skrivs den ut
        }
    }
}
